...
namespace CardEase
{
    public abstract class CardGroupManagerVertical<T> : MonoBehaviour, IPointerEnterHandler where T : CardModel
    {
        [Header("--------------Prefabs----------------")]
        ...
        public virtual void RefreshCardGroup()
        {
            ...
            //get the height of card and card zone
            float cardHeight = transform.GetChild(0).GetComponent<RectTransform>().rect.height;
            float cardZoneHeight = transform.parent.GetComponent<RectTransform>().rect.height;
            ...
            float groupSpacing = (cardZoneHeight / totalGroupCount) * 0.1f;
            cardZoneHeight = cardZoneHeight - (groupSpacing * (totalGroupCount + 1));

            //update spacing of group by card count
            float desiredSpacing = (cardZoneHeight / totalCardCount) - cardHeight;
            desiredSpacing = Mathf.Clamp(desiredSpacing, minSpacing, maxSpacing);
            verticalLayoutGroup.spacing = desiredSpacing;

            //update group size based on cards
            Vector2 sizeDelta = transform.GetComponent<RectTransform>().sizeDelta;
            sizeDelta.y = (cardHeight + desiredSpacing) * (transform.childCount - 1) + cardHeight;
